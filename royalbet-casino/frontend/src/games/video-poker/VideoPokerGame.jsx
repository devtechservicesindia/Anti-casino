...
function evaluate(hand) {
  ...
  if (counts[0] === 2) {
    ...
    if (pair >= 9) return 'Jacks or Better';
  }
  return null;
}
...
export default function VideoPokerGame() {
  ...
  async function dealHand() {
    ...
    const ok = await spend(bet, 'VIDEO_POKER');
    if (!ok) return;
    const d = mkDeck();
    setDeck(d.slice(5));
    setHand(d.slice(0, 5));
  ...
  async function drawCards() {
    const newDeck = [...deck];
    const newHand = hand.map((c, i) => held[i] ? c : newDeck.shift());
    setHand([...newHand]);
    const handName = evaluate(newHand);
    const multi = handName ? (PAYOUTS[handName] ?? 0) : 0;
    const payout = bet * multi;
    if (payout > 0) await earn(payout, 'VIDEO_POKER');
    setResult({ handName, multi, payout });
    setPhase('idle');
    if (multi >= 25) toast.success(`🃏 ${handName}! +${payout.toLocaleString()} 🪙`);
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#001a00] to-[#002600] text-white flex flex-col items-center px-4 py-6">
      ...
      <div className="w-full max-w-lg flex items-center justify-between mb-4">
        ...
        <h1 className="text-2xl font-black text-green-400 tracking-widest">VIDEO POKER 🃏</h1>
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="bg-black/50 border border-green-700/40 rounded-2xl p-4 mb-6">
          <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-xs">
            {Object.entries(PAYOUTS).map(([h, mul]) => (
              <div key={h} className={`flex justify-between ${result?.handName === h ? 'text-yellow-400 font-bold' : 'text-gray-400'}`}>
                <span>{h}</span><span>{mul}×</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {phase === 'idle' && (
          <button onClick={dealHand} disabled={balance < bet}
            className="w-full py-4 rounded-2xl bg-gradient-to-b from-green-500 to-green-700 text-black font-black text-2xl shadow-[0_4px_0_#14532d] hover:brightness-110 active:scale-95 transition disabled:opacity-40">
            DEAL 🃏
          </button>
        )}
        {phase === 'deal' && (
